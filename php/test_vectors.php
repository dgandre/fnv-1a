...
require_once("fnv-1a_32.inc.php");
require_once("fnv-1a_64.inc.php");
require_once("fnv-1a_128.inc.php");
require_once("fnv-1a_256.inc.php");
require_once("fnv-1a_512.inc.php");
require_once("fnv-1a_1024.inc.php");
...
$test_vectors_32 = array(	"" =>			"811c9dc5",
							"a" =>			"e40c292c",
							"b" =>			"e70c2de5",
							"c" =>			"e60c2c52",
							"d" =>			"e10c2473",
...
$test_vectors_64 = array(	"" =>			"cbf29ce484222325",
							"a" =>			"af63dc4c8601ec8c",
							"b" =>			"af63df4c8601f1a5",
							"c" =>			"af63de4c8601eff2",
							"d" =>			"af63d94c8601e773",
...
$test_vectors_128 = array(	"" =>			"6c62272e07bb014262b821756295c58d",
...
$test_vectors_256 = array(	"" =>			"dd268dbcaac550362d98c384c4e576ccc8b1536847b6bbb31023b4c8caee0535",
...
$test_vectors_512 = array(	"" =>			"b86db0b1171f4416dca1e50f309990acac87d059c90000000000000000000d21e948f68a34c192f62ea79bc942dbe7ce182036415f56e34bac982aac4afe9fd9",
...
$test_vectors_1024 = array(	"" =>			"",
...
{
	global $test_vectors_32;

	// The test pass unless we have at least one fail
	$failed = FALSE;

	// Go through test vectors
	foreach($test_vectors_32 as $string => $result) {
		if (fnv1a_32_str($string) != $result) {
			echo "fnv1a_32_str test failed for \"" . $string . "\"\n";
			echo fnv1a_32_str($string) . " returned\n";
	...
	// Check results
	if (FALSE == $failed) {
		echo "fnv1a_32_str successfully tested\n";
	}
}
...
{
	global $test_vectors_64;

	// The test pass unless we have at least one fail
	$failed = FALSE;

	// Go through test vectors
	foreach($test_vectors_64 as $string => $result) {
		if (fnv1a_64_str($string) != $result) {
			echo "fnv1a_64_str test failed for \"" . $string . "\"\n";
			echo fnv1a_64_str($string) . " returned\n";
			echo $result . " expected\n";
			$failed = TRUE;
		}
	}
	...
	// Check results
	if (FALSE == $failed) {
		echo "fnv1a_64_str successfully tested\n";
	}
}
...
{
	global $test_vectors_128;

	// The test pass unless we have at least one fail
	$failed = FALSE;

	// Go through test vectors
	foreach($test_vectors_128 as $string => $result) {
		if (fnv1a_128_str($string) != $result) {
			echo "fnv1a_128_str test failed for \"" . $string . "\"\n";
			echo fnv1a_128_str($string) . " returned\n";
			echo $result . " expected\n";
			$failed = TRUE;
		}
	}
	...
	// Check results
	if (FALSE == $failed) {
		echo "fnv1a_128_str successfully tested\n";
	}
}
...
{
	global $test_vectors_256;

	// The test pass unless we have at least one fail
	$failed = FALSE;

	// Go through test vectors
	foreach($test_vectors_256 as $string => $result) {
		if (fnv1a_256_str($string) != $result) {
			echo "fnv1a_256_str test failed for \"" . $string . "\"\n";
			echo fnv1a_256_str($string) . " returned\n";
			echo $result . " expected\n";
			$failed = TRUE;
		}
	}
	...
	// Check results
	if (FALSE == $failed) {
		echo "fnv1a_256_str successfully tested\n";
	}
}
...
{
	global $test_vectors_512;

	// The test pass unless we have at least one fail
	$failed = FALSE;

	// Go through test vectors
	foreach($test_vectors_512 as $string => $result) {
		if (fnv1a_512_str($string) != $result) {
			echo "fnv1a_512_str test failed for \"" . $string . "\"\n";
			echo fnv1a_512_str($string) . " returned\n";
			echo $result . " expected\n";
			$failed = TRUE;
		}
	}
	...
	// Check results
	if (FALSE == $failed) {
		echo "fnv1a_512_str successfully tested\n";
	}
}
...
{
	global $test_vectors_1024;

	// The test pass unless we have at least one fail
	$failed = FALSE;

	// Go through test vectors
	foreach($test_vectors_1024 as $string => $result) {
		if (fnv1a_1024_str($string) != $result) {
			echo "fnv1a_1024_str test failed for \"" . $string . "\"\n";
			echo fnv1a_1024_str($string) . " returned\n";
			echo $result . " expected\n";
			$failed = TRUE;
		}
	}
	...
	// Check results
	if (FALSE == $failed) {
		echo "fnv1a_1024_str successfully tested\n";
	}
}
